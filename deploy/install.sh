...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="PixelAPI 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 PixelAPI 服务："
    ...
    ["not_installed"]="PixelAPI 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 PixelAPI。"
    ...
    ["uninstall_complete"]="PixelAPI 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 PixelAPI"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 PixelAPI"
    ...
    ["server_config_desc"]="配置 PixelAPI 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="PixelAPI Installation Script"
    ...
    # Completion
    ["install_complete"]="PixelAPI installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start PixelAPI service:"
    ...
    ["not_installed"]="PixelAPI is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove PixelAPI from your system."
    ...
    ["uninstall_complete"]="PixelAPI has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install PixelAPI"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove PixelAPI"
    ...
    ["server_config_desc"]="Configure PixelAPI server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/pixelapi" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/pixelapi" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="pixelapi_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/pixelapi" "$INSTALL_DIR/pixelapi"
    chmod +x "$INSTALL_DIR/pixelapi"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/pixelapi"
}
...
User=pixelapi
Group=pixelapi
WorkingDirectory=/opt/pixelapi
ExecStart=/opt/pixelapi/pixelapi
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
SyslogIdentifier=pixelapi
...
ProtectHome=true
PrivateTmp=true
ReadWritePaths=/opt/pixelapi
RuntimeDirectory=pixelapi
...
start_service() {
    ...
    if systemctl start pixelapi; then
        print_success "$(msg 'service_started')"
        return 0
    else
        print_error "$(msg 'service_start_failed')"
        print_info "sudo journalctl -u pixelapi -n 50"
        return 1
    fi
}
...
enable_autostart() {
    ...
    if systemctl enable pixelapi 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status pixelapi"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u pixelapi -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart pixelapi"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop pixelapi"
    echo ""
    echo "=============================================="
}
...
# Upgrade function
upgrade() {
    # Check if PixelAPI is installed
    if [ ! -f "$INSTALL_DIR/pixelapi" ]; then
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/pixelapi" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    print_info "$(msg 'current_version'): $CURRENT_VERSION"

    # Stop service
    if systemctl is-active --quiet pixelapi; then
        print_info "$(msg 'stopping_service')"
        systemctl stop pixelapi
    fi

    # Backup current binary
    cp "$INSTALL_DIR/pixelapi" "$INSTALL_DIR/pixelapi.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/pixelapi.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/pixelapi"

    # Start service
    print_info "$(msg 'starting_service')"
    systemctl start pixelapi
...
install_version() {
    ...
    if [ -f "$INSTALL_DIR/pixelapi" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="pixelapi.backup.${current_version}"
        else
            backup_name="pixelapi.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/pixelapi" "$INSTALL_DIR/$backup_name"
...
uninstall() {
    ...
    print_info "$(msg 'stopping_service')"
    systemctl stop pixelapi 2>/dev/null || true
    systemctl disable pixelapi 2>/dev/null || true

    print_info "$(msg 'removing_files')"
    rm -f /etc/systemd/system/pixelapi.service
    systemctl daemon-reload
