...
import {
  ...
  useAppStore,
  useAuthStore,
  useConversationNotificationStore,
  useOnboardingStore,
} from "@/stores";
...
const flagChannelCenter = () =>
  flagAvailableChannels() || flagChannelMonitor();
...
//
// 条目顺序：密钥 → 用量 → 渠道中心 → 订阅/充值 → 资料。
// 渠道中心在同一页集中展示可用渠道、容量与运行状态。
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    {
      path: "/invoices",
      label: t("nav.invoiceManagement"),
      icon: OrderListIcon,
      hideInSimpleMode: true,
      featureFlag: flagInvoiceManagement,
    },
    {
      path: "/activities",
...
const personalNavItems = computed((): NavItem[] =>
  finalizeNav(buildSelfNavItems(false)),
);
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    },
    {
      ...
      label: t("nav.activityManagement"),
      icon: GiftIcon,
      hideInSimpleMode: true,
    },
    {
    ...
    },
    {
      ...
      children: [
        ...
        },
        {
          ...
          label: t("nav.storeCards"),
          icon: TicketIcon,
        },
        {
          path: "/admin/store/file-storage",
          label: t("nav.storeFileStorage"),
          icon: ServerIcon,
        },
      ],
    },
    {
      ...
      children: [
        ...
        },
        {
          ...
          label: t("nav.withdrawalManagement"),
          icon: CreditCardIcon,
          featureFlag: flagWithdrawalManagement,
        },
        {
          path: "/admin/invoices",
          label: t("nav.invoiceManagement"),
...
function navUnreadCount(item: NavItem): number {
  if (item.path === "/admin/conversations")
    return conversationNotificationStore.adminUnreadCount;
  if (item.path === "/conversations")
    return conversationNotificationStore.userUnreadCount;
  return 0;
}
